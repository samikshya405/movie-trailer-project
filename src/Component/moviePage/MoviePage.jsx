    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (data.results && data.results[0] && data.results[0].key) {
          setVideo_key(data.results.filter(video=>video.type==="Trailer")[0].key);
        } else {
      ...
      <div className="d-flex gap-5 mt-5 customize ">
        ...
        <div className=" d-flex flex-column gap-2 poster-des p-2">
          ...
          <div><Button variant="secondary" onClick={()=>{
            setShowTrailer(true)
          }}>Watch trailer</Button></div>
          {
            showTrailer && (
              <>
              <div className="trailervideo w-100 backdrop-blur">
                <div className="text-end">
                <Button variant="btn-link text-white" onClick={()=>{
                  setShowTrailer(false)
                }}>close</Button></div>
              <iframe  width="100%" height="500" src={`https://www.youtube.com/embed/${video_key}?autoplay=1&mute=0`} frameBorder="0" allowFullScreen allow="accelerometer; autoplay;"></iframe>
          ...
          <div className="d-flex gap-3 customize" >
            <p>
              ...
              Status:<span className="text-secondary"> {movieDetail.status}</span>
              
            </p>
            <p>
              
              Released Date: <span className="text-secondary">{movieDetail.release_date}</span>
              
            </p>
            <p>
              {" "}
              Runtime:{" "} <span className="text-secondary">
              {(movieDetail.runtime / 60).toFixed(0)}hour{" "}
              {movieDetail.runtime % 60}min{" "} </span>
            </p>
          </div>
          <hr/>
        </div>
